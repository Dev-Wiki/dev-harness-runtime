...
后续 DSH plugin 增加 Worker 标记下的工具 guard，并将实际使用的 `dsh-tools@0.1.5-rc.2` 加入精确 peer 列表。新本地 tgz 在隔离 headless profile 安装后，一次合成 bash 网络调用被 guard 拒绝。再加入只返回哈希、不写文件的 `dhr_propose_text` 后，真实 rc.1 headless Session 成功调用该工具；另一次 bash 写入仍被拒绝。完整工具目录和逐 Task 授权尚未证明，见 [K6](../verification/K6.md)。
