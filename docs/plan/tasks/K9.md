# 任务 K9：Antigravity Plugin 与 Skills 打包

> 本文只维护该任务的实施与验收详情。状态、优先级、依赖、执行顺序和阻塞以 [Dashboard](../Dashboard.md) 为唯一权威来源。

## 背景与目标

从统一源码生成 antigravity 可安装产物，并提供对应格式校验和安装说明。

## 执行上下文

- **权威需求**：[设计文档](../../design/runtime-design.md) §19.5、§29、§31、§41、§45 K9；[资料评估](../Readiness.md)。
...
- **包含**：Agent Plugin ZIP、项目级 .agents/skills/ 与 global-skills 兼容输出；doctor / probe 明确区分打包能力和执行能力；静态 validator 与 golden tree。
...
- `build/targets/antigravity.ts`
- `tests/fixtures/expected/antigravity/`
...
| 验证项 | 命令 / 操作 | 结果 / 证据链接 |
|---|---|---|
| 本任务验收 | 真实来源生成 / 静态校验 / 三包打包；`agy plugin validate/install/list/uninstall` | 离线与宿主安装 / 发现 / 卸载通过；未调用模型 Skill，见 [K9 验证记录](../../verification/K9.md)。 |
| 共享回归 | `pnpm build`、`pnpm harness:quick`、`node --test tests/packaging/*.test.mjs` | 均通过；打包专项 46/46，0 跳过；全量按用户要求留待开发完成。 |
...
- **未决问题**：Google Plugin 教程及 Portable 1.0.0 已取证，本机 `agy` 1.0.0 已安装 / 发现 / 卸载真实包；最低宿主版本、项目级 plugin 机制与模型会话内 Skill 调用仍缺。全局独立 Skill 路径为 ~/.gemini/config/skills/。
- **停止条件**：需要扩大范围、改变公共语义或验收、使用无证据宿主能力，或发现 Git / Planning 外部漂移时停止实现，回到 Dashboard 对齐。

---

*最后更新：2026-09-17（三产物与原生安装链通过；会话 Skill 调用未验证）*
