# 任务 K8：OpenCode npm 与本地插件打包

> 本文只维护该任务的实施与验收详情。状态、优先级、依赖、执行顺序和阻塞以 [Dashboard](../Dashboard.md) 为唯一权威来源。

## 背景与目标

从统一源码生成 opencode 可安装产物，并提供对应格式校验和安装说明。

## 执行上下文

- **权威需求**：[设计文档](../../design/runtime-design.md) §19.4、§29、§31、§41、§45 K8；[资料评估](../Readiness.md)。
- **平台输入**：[R1 平台基线](../../integration/PLATFORM_BASELINE.md)、[fixture 与离线命令](../../../tests/fixtures/platform-specs/README.md)。
- **代码入口**：从下列影响文件进入。初始化时目标项目为空，所列实现与测试路径均为建议新建路径，不能当作已有代码。
- **相关测试**：最小 fixture 与真实生成目录逐项比较；解包后检查引用、Skill 和 runtime 资源；宿主可用时隔离安装、发现、调用 smoke、卸载。
- **必须保持的不变量**：业务流程保持共享；最终包不依赖开发机绝对路径；未通过 probe 的能力不启用。


## 范围

- **包含**：npm tgz 与本地 .opencode/plugins/、.opencode/skills/ ZIP 两种产物；doctor / probe 明确区分打包能力和执行能力；静态 validator 与 golden tree。
- **不包含**：增加独立 Orchestrator；未经实测启用自动 Session 编排；自动提交 Marketplace。

## 影响文件

以下路径以 `dev-harness-runtime/` 为根；执行前核对已建立的目录与命令，已有文件按职责更新。

- `packages/adapter-opencode/src/`
- `packages/adapter-opencode/tests/`
- `build/targets/opencode.ts`
- `tests/fixtures/expected/opencode/`

## 建议实施顺序

1. 重读 Dashboard 与本任务，检查来源是否漂移；从资料评估对应条目和已形成的决策记录确认输入。
2. 先建立本任务验收所列的正常、异常和边界样例，再在范围内实现或完成决策取证；不为迁就实现修改公共协议。
3. 按下列验收逐项验证，记录真实命令、环境、结果和稳定证据；更新相关事实文档，再按 Planning 生命周期收口。

## 验收标准

- [x] 产物树、manifest、版本、依赖、Skill 数量和资源引用通过静态与 golden 验证。
- [x] 缺少必需文件、非法字段、绝对本机路径、重复 Skill 和版本不一致均失败。
- [x] 安装说明使用原生机制；OpenCode 1.18.31 对本地变体的插件与 Skill 发现、移除后消失，和离线校验分别报告。
- [ ] npm 变体宿主加载及模型会话内 Skill 调用通过；仅有发现证据不能代替调用。

## 验证证据

| 验证项 | 命令 / 操作 | 结果 / 证据链接 |
|---|---|---|
| 本任务验收 | 真实来源生成 / 静态校验 / 双归档；npm 临时离线安装和包内 CLI；OpenCode 1.18.31 本地插件发现 / 移除 | 离线 3/3 与本地变体宿主发现 / 移除通过；npm 变体及模型调用未验，见 [K8 验证记录](../../verification/K8.md)。 |
| 共享回归 | `pnpm build`、`pnpm harness:quick`、`node --test tests/packaging/*.test.mjs` | 均通过；打包专项 43/43，0 跳过；全量按用户要求留待开发完成。 |

## 已确认决策

- 项目名 `dev-harness-runtime`，CLI 全部使用 `dhr`。
- 状态根目录统一为 `$(git rev-parse --git-path dev-harness-runtime)/runs/`；具体布局以设计 §10 为准。
- 业务流程保持共享；最终包不依赖开发机绝对路径；未通过 probe 的能力不启用。

## 未知项与停止条件

- **已知风险**：来源升级、接口不完整或环境不足可能使验收不可复现；不能将设计示例或历史通过记录当作本次运行证据。
- **未决问题**：npm tgz 可离线安装，双产物布局已验证；OpenCode 1.18.31 已发现本地变体的插件和三个 Skill 并确认删除后消失。npm 变体宿主加载、插件 hook 与模型会话内 Skill 调用仍缺，Executor 另行评估。
- **停止条件**：需要扩大范围、改变公共语义或验收、使用无证据宿主能力，或发现 Git / Planning 外部漂移时停止实现，回到 Dashboard 对齐。

---

*最后更新：2026-09-17（本地变体宿主发现与移除通过；模型调用及 npm 宿主加载待验）*
