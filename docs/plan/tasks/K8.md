...
- [x] 安装说明使用原生机制；OpenCode 1.18.31 对本地变体的插件与 Skill 发现、移除后消失，和离线校验分别报告。
- [ ] npm 包名自动安装、插件 hook 与模型会话内 Skill 调用通过；仅有本地包内入口发现证据不能代替调用。

## 验证证据

| 验证项 | 命令 / 操作 | 结果 / 证据链接 |
|---|---|---|
| 本任务验收 | 真实来源生成 / 静态校验 / 双归档；npm 临时离线安装和包内 CLI；OpenCode 1.18.31 双变体入口发现、本地变体移除 | 离线 3/3、双变体包内入口发现及本地变体移除通过；npm 包名自动安装和模型调用未验，见 [K8 验证记录](../../verification/K8.md)。 |
...
- **未决问题**：npm tgz 可离线安装，双产物布局已验证；OpenCode 1.18.31 已发现双变体插件入口与单独安装的三个 Skill，并确认本地变体删除后消失。未发布包无法验证按 npm 包名自动安装；插件 hook 与模型会话内 Skill 调用仍缺，Executor 另行评估。
- **停止条件**：需要扩大范围、改变公共语义或验收、使用无证据宿主能力，或发现 Git / Planning 外部漂移时停止实现，回到 Dashboard 对齐。

---

*最后更新：2026-09-17（本地变体宿主发现与移除通过；模型调用及 npm 宿主加载待验）*
