...
- **代码入口**：packages/core/src/state 与 lock 已实现；对外导出由 core/src/index.ts 汇总。RunState / LockMetadata 静态校验沿用 contracts，跨模块隔离见 tests/integration/private-state.test.mjs。
...
- `tests/integration/private-state.test.mjs`、core 导出、公共事实文档与本任务归档。
...
- [x] 并发启动最多一个 owner；CAS 冲突不会覆盖较新状态。
- [x] 每个 Run 仅以 `<run-id>/run.json` 为权威状态文件；同级 attempts/ 保存日志、results/ 保存执行结果、summary.json 保存派生摘要，不形成第二份 Run 状态。
- [x] 主仓与 linked worktree 独立；Windows / Linux / WSL 的差异被实测或明确报告。
- [x] 中断后可识别完整状态或可恢复旧状态；stale lock 处理遵循明确 owner 校验。

## 验证证据

| 验证项 | 命令 / 操作 | 结果 / 证据链接 |
|---|---|---|
| 本任务验收 | 多进程写入争用、revision 冲突、半写与进程退出故障注入、linked worktree 隔离。 | 锁 7 项、状态 21 项和跨模块集成 2 项通过，包含真实多进程与 SIGKILL，见 [K3-L 验证记录](../../../verification/K3-L.md)。 |
| 共享回归 | 采用 Dashboard 的共享验证基线与届时 HARNESS 已验证入口 | pnpm verify 通过：289 项 Node、21 份 Schema、10 项平台检查和 CLI 安装；最后测试清理变更定向复跑 21 项通过。 |
...
*最后更新：2026-09-17（完成私有状态、原子写入与互斥验收）*
