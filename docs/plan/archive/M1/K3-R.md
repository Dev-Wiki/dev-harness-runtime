...
- **代码入口**：core/recovery 已提供恢复、显式对齐与新 Run 门禁；state 补持锁诊断读取 / 枚举、初始快照初始化和确定证据复用。RunState.repoIdentity 固定创建身份，当前 Git 边界读取 acceptedSnapshot。
- **相关测试**：在 execute、result、计划收口、commit、state persist 边界注入中断，验证新的执行身份和恢复决策；真实宿主 Session 由后续 Adapter 任务验证。
...
- `docs/RECOVERY.md`
- `packages/core/src/state/` 与对应初始化测试、Core 导出、lock guard 清理函数、公共事实文档和本任务归档。
...
- [x] 每个可中断阶段都有恢复、拒绝或人工处理的明确结果。
- [x] HEAD / 计划 / dirty paths / Run 版本不匹配时拒绝并保留原始证据。
- [x] 重复 resume 不重复提交或领取已完成任务；新 attempt 与旧证据可区分。
- [x] 未接受的归档在新 Run 中同样触发 PENDING_RECONCILIATION；显式对齐只记录验证过的处置，不改写失败历史或 Planning。

## 验证证据

| 验证项 | 命令 / 操作 | 结果 / 证据链接 |
|---|---|---|
| 本任务验收 | 在 execute、result、计划收口、commit、state persist 边界注入中断，验证新的执行身份和恢复决策；真实宿主 Session 由后续 Adapter 任务验证。 | 受影响测试 144 项通过，详见 [K3-R 验证记录](../../../verification/K3-R.md)。 |
