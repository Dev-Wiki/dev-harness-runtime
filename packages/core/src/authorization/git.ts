...
import { isRepoPath, parseContract, type EvidenceRef, type RunState, type TaskExecutionRequest, type TaskExecutionResult } from '@dev-harness-runtime/contracts';
import { withLock, type LockHandle } from '../lock/index.js';
import { consumeAcceptedTask, type AcceptedTaskCapability } from '../result/acceptance.js';
...
export async function commitAcceptedTask(handle: LockHandle, capability: AcceptedTaskCapability, options: CommitAcceptedTaskOptions): Promise<CommittedTask> {
  ...
  try {
    ...
    for (const path of paths) {
      ...
      requireValue((await assertSupportedCommitProject(root, options.gitBinary)).fingerprint === supported.fingerprint, 'DRIFT_DETECTED', 'Git policy configuration changed before staging');
      await withLock(handle, async (owner) => { await owner.assertOwner(); await git(options.gitBinary, root, [...prefix, 'add', '--', path], { env: executionEnv }); });
    }
    const staged = await recaptureSnapshot(before);
    ...
    const actualTree = await withLock(handle, async (owner) => { await owner.assertOwner(); return text(options.gitBinary, root, [...prefix, 'write-tree'], executionEnv); });
    ...
    requireValue((await assertSupportedCommitProject(root, options.gitBinary)).fingerprint === supported.fingerprint, 'DRIFT_DETECTED', 'Git policy configuration changed before commit');
    await writeFile(join(temporary, 'message'), approved.message);
    await withLock(handle, async (owner) => { await owner.assertOwner(); await git(options.gitBinary, root, [...prefix, 'commit', '--quiet', '--no-edit', '--no-status', '--cleanup=verbatim', '--file', join(temporary, 'message')], { env: executionEnv }); });
    requireValue((await assertSupportedCommitProject(root, options.gitBinary)).fingerprint === supported.fingerprint, 'DRIFT_DETECTED', 'Git policy configuration changed during commit');
