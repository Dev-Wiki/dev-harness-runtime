...
export interface CliOptions { cwd?: string; signal?: AbortSignal; services?: RuntimeServices }
const knownAdapters = ['codex', 'dsh', 'cursor', 'opencode', 'antigravity'];
const commands = ['doctor', 'status', 'run', 'resume', 'reconcile'];
...
已知 Adapter: ${knownAdapters.join(', ')}。当前分发包没有已实现的宿主 Executor；
...
function parse(args: readonly string[], cwd: string): Parsed {
  ...
  const common = ['--project', '--docs-root'];
...
function checkAdapter(id: string, services?: RuntimeServices): void {
  if (!knownAdapters.includes(id) && !services?.adapters.list().some((adapter) => adapter.id === id)) {
    throw new CliError('UNKNOWN_ADAPTER', `未知 Adapter: ${id}`);
  }
}
...
export async function runCli(args: readonly string[], output: CliOutput, options: CliOptions = {}): Promise<number> {
  ...
  try {
    ...
    if (parsed.adapter !== undefined) checkAdapter(parsed.adapter, options.services);
    ...
    if (parsed.command === 'doctor') {
      ...
      const ids = parsed.adapter === undefined ? knownAdapters : [parsed.adapter];
      const adapters = ids.map((id) => ({ id, available: false, authorizationEnforced: false,
        reason: options.services?.adapters.list().some((adapter) => adapter.id === id) ? 'Registered services have not been probed by this read-only diagnostic' : 'No implemented host Executor is installed' }));
    ...
    const result = parsed.command === 'run'
      ? await startRuntimeRun({ ...common, adapter: parsed.adapter!, selection: parsed.selection!, commit: parsed.commit }, options.services)
      : parsed.command === 'resume'
        ? await resumeRuntimeRun({ ...common, runId: parsed.runId!, expectedRevision: parsed.expectedRevision! }, options.services)
        : await reconcileRuntimeRun({ ...common, runId: parsed.runId!, expectedRevision: parsed.expectedRevision!, resolutionRef: await resolution(parsed.resolutionPath!) }, options.services);
    const project = await discoverProject(parsed.cwd, discovery);
